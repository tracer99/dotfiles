...
# Prompt: Starship when available, custom PS1 fallback
case "$TERM" in
xterm*|rxvt*)
...
if grep -qiE 'microsoft|wsl' /proc/version 2>/dev/null && command -v ssh.exe &>/dev/null; then
  _has_1password=0
  for _1p in \
    "/mnt/c/Program Files/1Password/1Password.exe" \
    "/mnt/c/Program Files/1Password/app/1Password.exe" \
    /mnt/c/Users/*/AppData/Local/Programs/1Password/1Password.exe \
    /mnt/c/Users/*/AppData/Local/Microsoft/WindowsApps/op.exe \
    /mnt/c/Users/*/AppData/Local/Microsoft/WindowsApps/op-ssh-sign-wsl.exe; do
    [ -e "$_1p" ] || continue
    _has_1password=1
    break
  done
  if [ "$_has_1password" -eq 1 ]; then
    alias ssh='ssh.exe'
    alias ssh-add='ssh-add.exe'
  fi
  unset _has_1password _1p
fi
...
[ -x '/mnt/c/Program Files (x86)/Google/Chrome/Application/chrome.exe' ] && \
  export LAUNCHPAD_CHROME='/mnt/c/Program Files (x86)/Google/Chrome/Application/chrome.exe'
[ -x '/mnt/c/Program Files/Mozilla Firefox/firefox.exe' ] && \
